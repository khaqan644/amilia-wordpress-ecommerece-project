...
<div class="cms-progress-wraper <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    <div class="cms-progress-body">
        <div class="<?php echo esc_attr($item_class. ' '.$layout );?>" <?php echo esc_attr(($layout == 'style2') ? 'data-percent="'.$value.'" ': ''); ?>>
            ...
            <div class="cms-progress progress<?php if($vertical){ echo ' vertical bottom'; } ?> <?php if($striped){echo ' progress-striped';}?>" 
                ...
                <?php if ($layout == 'style1') : ?>
                    <div id="item-<?php echo esc_attr($atts['html_id']); ?>" 
                        ...
                        >
                        <div class="cms-progress-title" style="<?php echo esc_attr(($height) ? 'height:'.$height.'; line-height:'.$height : ''); ?>">
                            <?php if($item_title):?>
                ...
                <?php elseif ($layout == 'style2') : ?>
                    <?php if($item_title):?>
                        <div class="cms-progress-title">
                            <span style="<?php echo esc_attr(($height) ? 'height:'.$height.'; line-height:'.$height : ''); ?>"><?php echo apply_filters('the_title',$item_title);?></span>
                        </div>
                    <?php endif;?>
                    <?php
                    ...
                    <?php if($show_value): ?>
                        <div class="csm-bar-percent" style="<?php echo esc_attr(($height) ? 'height:'.$height.'; line-height:'.$height : ''); ?>"><?php echo esc_attr($value.$value_suffix);?></div>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        </div>
...
<?php elseif ($layout == 'style3') : ?>
    <div class="progress <?php echo esc_attr($layout); ?> <?php echo esc_attr(($striped) ? 'progress-striped' : ''); ?>">
        <div style="width: <?php echo esc_attr($value); ?>%;" 
            aria-valuemax="100" 
            aria-valuemin="0" 
            aria-valuenow="<?php echo esc_attr($value); ?>" 
            role="progressbar" class="progress-bar <?php echo ''.$progressbar_bt_type; ?> <?php echo esc_attr(($show_value) ? 'show_value' : ''); ?>">
